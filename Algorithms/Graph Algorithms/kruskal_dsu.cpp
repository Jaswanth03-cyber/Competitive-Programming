...
class disjoint_set{
    ...
    public:
    disjoint_set(int n){
        parent.resize(n, -1) ; // be very carefull because we generally keep this as resize(n, 0) but there is a problem as for 
        // some i src = 0 so while finding set when parent[0] = 0 will be a problem
        rank.resize(n, -1) ;
    }
